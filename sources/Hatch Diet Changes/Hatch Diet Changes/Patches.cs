    ...
    public class Patches
    {
        ...
        public class RegularHatchPatch
        {
                ...
                ref List<Diet.Info> __result)
            {
                ...
                foreach (Tuple<Tag, Tag> T in Setup.regularHatchMetals())
                {
                    ...
                    {
                        T.first
                    }), T.second, caloriesPerKg * conv.c100kg, 0.95f, diseaseId, diseasePerKgProduced));
                }
            }
        }
        ...
        public class VeggiePatch
        {
            public static void Postfix(
                ...
                ref List<Diet.Info> __result)
            {
                foreach (Tag t in Setup.veggieHatchCarbon())
                {
        ...
        public class StoneHatch_RemoveObsidian_AddGlass_AddKatairite
        {
                ...
                ref List<Diet.Info> __result)
            {
                ...
                foreach (var t in Setup.stoneHatchHalf())
                {
                    __result.Add(new Diet.Info(
                        new HashSet<Tag>() { t.first },
                        t.second,
                        caloriesPerKg,
                        producedConversionRate,
                        diseaseId,
                ...
                var fossil = Setup.stoneHatchQuarter();
                __result.Add(new Diet.Info(
                    new HashSet<Tag>() { fossil.first },
                    fossil.second,
                    caloriesPerKg * conv.c100kg,
                    producedConversionRate / 2f,
                    diseaseId,
                    diseasePerKgProduced));
            }
        }
        ...
        public class SmoothHatch_AddRefinedCarbon
        {
            public static void Postfix(
                ...
                ref List<Diet.Info> __result)
            {
                foreach (var t in Setup.smoothHatchExtras())
                {
                    __result.Add(new Diet.Info(
                        new HashSet<Tag>() { t.first },
                        t.second,
